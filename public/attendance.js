...
async function initAttendance() {
    ...
    try {
        attendanceUser = await fetch('/api/auth/me').then(r => r.json());
        ...
        attShowError('Failed to initialize attendance: ' + e.message);
    }
}
...
function renderAttendanceWidget() {
    ...
    const duration = activeSession ? attCalculateDuration(activeSession.clockInTime) : '00:00:00';
...
function showClockInModal() {
    ...
    const modal = attShowModal(modalContent);
...
async function clockIn() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            attShowError(error.error || 'Failed to clock in');
            if (btn) {
                btn.disabled = false;
        ...
        attShowError('Failed to clock in');
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = 'Clock In Now';
        }
...
async function clockOut() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            renderAttendanceWidget();
            await loadAttendanceHistory();
            attShowSuccess(`Clocked out! Work duration: ${attFormatDuration(session.workDuration)}`);
        } else {
            const error = await response.json();
            attShowError(error.error || 'Failed to clock out');
            if (btn) {
                btn.disabled = false;
        ...
        attShowError('Failed to clock out');
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = 'Clock Out Now';
        }
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        if (activeSession) {
            const currentSessionDuration = attCalculateMinutes(activeSession.clockInTime);
            const totalDuration = (window.todayBaseMinutes || 0) + currentSessionDuration;

            const durationDisplay = document.getElementById('duration-display');
            const summaryDuration = document.getElementById('summary-duration');

            if (durationDisplay) durationDisplay.textContent = attFormatDuration(currentSessionDuration); // Show current session
            if (summaryDuration) summaryDuration.textContent = attFormatDuration(totalDuration); // Show DAY total
        }
    }, 1000);
}

// Calculate minutes duration
function attCalculateMinutes(startTime) {
...
function attCalculateDuration(startTime) {
    const start = new Date(startTime);
    const now = new Date();
    const diff = now - start;
...
function attFormatDuration(minutes) {
...
async function loadAttendanceHistory() {
    ...
    try {
        ...
        if (summaryDuration && !activeSession) {
            summaryDuration.textContent = attFormatDuration(totalMinutes);
        }
...
function renderAttendanceHistory(records) {
    ...
    tbody.innerHTML = records.map(record => {
        ...
        const duration = record.workDuration ? attFormatDuration(record.workDuration) : 'In Progress';
...
function renderTodayAttendance(records) {
    ...
    tbody.innerHTML = records.map(record => {
        const clockIn = new Date(record.clockInTime);
        const clockOut = record.clockOutTime ? new Date(record.clockOutTime) : null;
        const duration = record.workDuration ? attFormatDuration(record.workDuration) : attCalculateDuration(record.clockInTime);
...
function attCloseModal(modal) {
    modal.remove();
}

function attShowSuccess(message) {
    alert(message);
}

function attShowError(message) {
    alert('Error: ' + message);
}
