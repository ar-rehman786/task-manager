...
function DashboardContent() {
  ...
  const { data: attendanceStatus } = useQuery<AttendanceStatus | null>({
  ...
  const myTasks = useMemo(
    () => tasks.filter((t: any) => t.assignedUserId === user?.id),
    [tasks, user?.id],
  );
  ...
  const hoursToday = useMemo(() => {
    let totalMinutes = 0;
    if (attendanceStatus?.status === "active" && attendanceStatus.clockInTime) {
      const start = new Date(attendanceStatus.clockInTime);
      const diffMs = now.getTime() - start.getTime();
      totalMinutes = Math.floor(diffMs / 60000);
    } else {
      totalMinutes = attendanceStatus?.workDuration || 0;
    }
    return Math.floor(totalMinutes / 60);
  }, [attendanceStatus, now]);
    ...
    () =>
      users.map((u: any) => ({
        ...
        activeProjects: projects.filter(
          (p: any) => p.assignedUserId === u.id && p.status === "active",
        ),
      })),
    [users, projects],
  );

  const pendingAccessProjects = useMemo(
    () =>
          ...
          <Card className="p-6 hover:shadow-lg transition-all cursor-pointer border-transparent hover:border-primary/20">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-2">
                  {milestones.filter((m: any) => m.status !== "done").length}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  in progress
                </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3">
            {teamWorkload.filter((u: any) => u.activeProjects.length === 0)
              .length > 0 ? (
              teamWorkload
                .filter((u: any) => u.activeProjects.length === 0)
                .map((u: any) => (
                  <div
                    key={u.id}
                    className="flex items-center justify-between p-3 rounded-lg hover:shadow-sm hover:bg-muted/30 transition-all duration-150"
                  >
        ...
        <Card className="p-6">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
            <Circle className="w-3 h-3 fill-red-500 text-red-500" /> Busy Team
            Members
          </h2>
            ...
            {teamWorkload.filter((u: any) => u.activeProjects.length > 0)
              ...
              teamWorkload
                ...
                .map((u: any) => (
                  <div
                    key={u.id}
                    className="p-3 rounded-lg hover:shadow-sm hover:bg-muted/30 transition-all duration-150"
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground uppercase tracking-tight">
                            {u.activeProjects.length} Active Project
                            {u.activeProjects.length !== 1 ? "s" : ""}
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-3 ml-2">
                      {u.activeProjects.map((p: any) => (
                        <div
                          key={p.id}
                          className="flex flex-col gap-2 p-2 bg-muted/20 rounded-md"
                        >
              ...
              <p className="text-muted-foreground text-sm italic">
                No one is currently working on active projects.
              </p>
            )}
          </div>
        </Card>
      ...
      {pendingAccessProjects.length > 0 && (
        <Card className="p-6 border-red-200 bg-red-50/10">
          <h2 className="text-xl font-semibold mb-4 text-red-700 flex items-center gap-2">
            <AlertTriangle className="w-5 h-5" /> Pending Access Requests
          </h2>
          ...
          {recentTasks.length > 0 ? (
            recentTasks.map((task: any) => (
              <Link key={task.id} href="/tasks" className="block group">
                <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg group-hover:bg-muted transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div
      ...
      <Card className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <Flag className="w-5 h-5 text-primary" /> Upcoming Milestones
          </h2>
          <Link
            href="/projects"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {milestones
            ...
            .map((m: any) => (
              <div
                key={m.id}
                className="border rounded-xl p-4 bg-muted/20 hover:bg-muted/30 transition-colors"
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold truncate" title={m.title}>
                    {m.title}
                  </h3>
                  <Badge
                    variant="outline"
                    className="text-[10px] h-4 bg-primary/10 text-primary border-0"
                  >
                ...
                <div className="space-y-1.5 mb-4">
                  ...
                  <div className="h-1.5 w-full bg-primary/10 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${m.totalTasks > 0 ? (m.completedTasks / m.totalTasks) * 100 : 0}%`,
                      }}
                    />
                  </div>
                ...
                <div className="flex justify-between items-center mt-auto pt-2 border-t">
                  <div className="text-xs">
                    ...
                    <span
                      className={`font-medium ${new Date(m.dueDate) < new Date() ? "text-red-500" : "text-foreground"}`}
                    >
                  ...
                  <Link
                    ...
                    className="text-[10px] uppercase font-bold text-primary hover:underline flex items-center gap-0.5"
                  >
